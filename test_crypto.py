...
def derive_wallet(mnemonic: str):
    ...
    return {mnemonic, address, xpub, xprv, wif}

def generate_mnemonic(strength: str=128):
...
def main():
    my_mnemonic = generate_mnemonic()
    my_wallet = create_wallet(my_mnemonic, "mypassword")

def main():
    from hdwallet.derivations import BIP44Derivation
    mnemonic = generate_mnemonic()
    print("Mnemonic:", mnemonic)
    wallet = create_wallet(mnemonic, "mypassword")

    # List to hold multiple addresses
    addresses = []

    # Loop through indexes 0 to 4
    for i in range(5):
        derivation = BIP44Derivation(coin_type=175, account=0, change=0, address=i)
        wallet.update_derivation(derivation)
    ...
    # Optionally dump full wallet info
    print(wallet.dump())
